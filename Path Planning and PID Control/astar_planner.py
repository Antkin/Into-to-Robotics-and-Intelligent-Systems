...
class AStarPlanner(object):
    ...
    def plan(self, start_state, dest_state):
        ...
        #Since Q is our priority queue we set the source node to have a priority equal to the heuristic at the source node
        #Heuristic uses euclidean distance to calculate distance between curr node and dest node
        ...
        while Q:
            ...
            for ns in self.get_neighboring_states(s):
                ...
                if (ns not in Q) or (alternative_best_dist_ns < best_dist_found[ns.x, ns.y]):
                    #Only change here is that we add the heuristic to the best_distance when updating the priority queue
                    #Q[ns] = alternative_best_dist_ns
                    Q[ns] = alternative_best_dist_ns + sqrt((ns.x - dest_state.x)**2 + (ns.y - dest_state.y)**2)
                    #Best distance found remains unchanged as this does not take into account the heuristic
...
if __name__ == "__main__":
    ...
    print('Planning complete in '+str(end_time-start_time)+'. See image astar_result.png to check the plan.')
