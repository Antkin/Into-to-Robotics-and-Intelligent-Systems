...
class RRTPlanner(object):
    ...
    def steer_towards(self, s_nearest, s_rand, max_radius):
        ...
        #First we will check if the distance between s_rand and s_nearest is within the desired radius
        distance_apart = sqrt((s_nearest.x - s_rand.x)**2 + (s_nearest.y - s_rand.y)**2)
        if distance_apart < max_radius:
            return State(s_rand.x, s_rand.y, s_nearest)
        
        #If the distance is not within our radius, we will put the point on the line from s_nearest
        #to s_rand at a distance of maxradius
        distance_ratio = max_radius / distance_apart
        x = ((1 - distance_ratio)*s_nearest.x) + (distance_ratio*s_rand.x)
        y = ((1 - distance_ratio)*s_nearest.y) + (distance_ratio*s_rand.y)
        #Convert back to integer before passing it back as some of the functions dont accept float
        x = int(x)
        y = int(y)
    ...
    def path_is_obstacle_free(self, s_from, s_to):
        ...
        distance_apart = sqrt((s_from.x - s_to.x)**2 + (s_from.y - s_to.y)**2)
        if distance_apart == 0:
            return False
        ...
        for i in range(max_checks):
            ...
            #We calculate a ratio based on how far along the line we want to check
            distance_ratio = ((float(i)/max_checks)*distance_apart) / distance_apart
            x = ((1 - distance_ratio)*s_from.x) + (distance_ratio*s_to.x)
            y = ((1 - distance_ratio)*s_from.y) + (distance_ratio*s_to.y)
            #Need to convert back to integer before passing it
            x = int(x)
            y = int(y)
    ...
    def plan(self, start_state, dest_state, max_num_steps, max_steering_radius, dest_reached_radius,live_view=True):
        ...
        for step in range(max_num_steps):
            ...
            #Find the nearest point to that state
            s_nearest = self.find_closest_state(tree_nodes, s_rand)
            
            #s_new is the new point created. We will attempt to connect 
            #s_nearest to this point.
...
if __name__ == "__main__":
    ...
    dest_state = State(250, 200, None)

    max_num_steps = 1200     # max number of nodes to be added to the tree 
