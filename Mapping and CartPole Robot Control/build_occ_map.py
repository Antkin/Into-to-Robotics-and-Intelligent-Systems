...
class HuskyMapper:
    ...
    def from_laser_to_map_coordinates(self, points_in_baselaser_frame):
        ...
        points_in_map_frame = []
        for (x,y,z) in points_in_baselaser_frame:
            points_in_map_frame.append([x-20, y-10, 0])
    ...
    def is_in_field_of_view(self, robot_row, robot_col, laser_theta, row, col):
        ...
        distance = sqrt((row - robot_row)**2 + (col - robot_col)**2) * self.ogm.meters_per_cell
        if distance > self.max_laser_range and distance < self.min_laser_range: return False
        
        #Find angle between robot and cell w.r.t. horizontal axis
        delta_x = col - robot_col
        delta_y = row - robot_row
        theta_radians = atan2(delta_y, delta_x)
        
        #Determine if this angle is within the LIDAR range
        angle_difference = atan2(sin(theta_radians - laser_theta), cos(theta_radians - laser_theta))
        
        if angle_difference < self.max_laser_angle and angle_difference > self.min_laser_angle:
            return True
    ...
    def inverse_measurement_model(self, row, col, robot_row, robot_col, robot_theta_in_map, beam_ranges, beam_angles):
        ...
        #
        r = self.ogm.meters_per_cell * sqrt((row - robot_row)**2 + (col - robot_col)**2)
        
        delta_x = col - robot_col
        delta_y = row - robot_row
        theta_radians = atan2(delta_y, delta_x)
        diff_angle = atan2(sin(theta_radians - robot_theta_in_map), cos(theta_radians - robot_theta_in_map))
    ...
    def process_laser_scan(self, ranges_in_baselaser_frame):
        ...
        for i in range(len(ranges_in_baselaser_frame)):
            points_xyz_in_baselaser_frame.append([ranges_in_baselaser_frame[i]*cos(self.angles_in_baselaser_frame[i]), ranges_in_baselaser_frame[i]*sin(self.angles_in_baselaser_frame[i]), 0])
